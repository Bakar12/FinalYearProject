...
def change_password():
    if request.method == 'POST':
        ...
        conn = sqlite3.connect(DATABASE)
        cursor = conn.cursor()
        cursor.execute('SELECT Password FROM Admins WHERE AdminID = ?', (session['user_id'],))
        ...
        conn = sqlite3.connect(DATABASE)
        cursor = conn.cursor()
        cursor.execute('UPDATE Admins SET Password = ? WHERE AdminID = ?', (new_password_hash, session['user_id']))
        conn.commit()
        cursor.close()
        conn.close()
